...
// Scales: Femur/Patella = 0.45, Tibia = 0.43, Fibula = 0.39
const scaleFemur = 0.45;
const scaleTibia = 0.43;
const scaleFibula = 0.39;
...
const mappedPoints = points.map(pt => {
  ...
  const Y = (pt.y - yCenter_user) * scaleFactor + 250;
  return {
    index: pt.index,
...
function replaceCoordsList(functionName, startIndex, endIndex) {
  ...
  // Find the unique array declaration string
...
console.log("SUCCESS: atlasData.ts coordinates updated correctly with Tibia = 0.43 and Fibula = 0.39!");
